...
describe('Environment variables – povinné pre Firebase + salon', () => {
  it('VITE_FIREBASE_PROJECT_ID je nastavený', () => {
    ...
    expect(projectId, 'Project ID stále používa placeholder hodnotu').not.toBe('your-project-id');
  });
  ...
  it('Firebase core env vars sú nastavené', () => {
    ...
    expect(import.meta.env.VITE_FIREBASE_API_KEY, 'VITE_FIREBASE_API_KEY stále používa placeholder hodnotu').not.toBe('your-api-key');
    expect(import.meta.env.VITE_FIREBASE_AUTH_DOMAIN, 'VITE_FIREBASE_AUTH_DOMAIN chýba').toContain("firebaseapp.com");
    expect(import.meta.env.VITE_FIREBASE_AUTH_DOMAIN, 'VITE_FIREBASE_AUTH_DOMAIN stále používa placeholder hodnotu').not.toContain('your-project-id');
    expect(import.meta.env.VITE_FIREBASE_APP_ID, 'VITE_FIREBASE_APP_ID chýba').toBeTruthy();
    expect(import.meta.env.VITE_FIREBASE_APP_ID, 'VITE_FIREBASE_APP_ID stále používa placeholder hodnotu').not.toBe('your-app-id');
  });
